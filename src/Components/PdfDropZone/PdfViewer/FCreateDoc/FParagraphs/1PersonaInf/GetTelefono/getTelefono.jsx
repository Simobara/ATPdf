...
const extractTelefono = (origText) => {
    const keywords = [
...
export const getTel = async (origText) => {
    let phoneNumber = await extractTelefono(origText);
    if (!phoneNumber) {
        phoneNumber = extractTelefonoWithNLP(origText);
    }
    return new Paragraph({
        alignment: "left",
        children: [
            new TextRun(`Telefono: ${phoneNumber ? phoneNumber : " / "}`)
        ]
    });
}
